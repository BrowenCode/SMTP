...
def getTo():
    ...
    while not isEnd(ignoreNullSpace(i)):
        c, i = equals(i, ',')
        if not c:
            return False
        i = ignoreNullSpace(i)
...
def getResponse(n=-1):
    ...
    try:
        if n != -1 and int(response[0:3]) != n:
            print('Bad SMTP receipt. Terminating program.') # if expected response code is provided and differs from actual response code, then quit.
            smtpquit()
    except ValueError:
        smtpquit()
...
def sendmail():
    ...
    clientSocket.send(("HELO " + platform.node() +
                       " pleased to meet you").encode())
    getResponse(250)
